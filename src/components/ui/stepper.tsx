...
interface StepperProps {
  ...
  suffix?: string;
  className?: string;
  /** Accessible name for the control (also specializes the +/- button labels). */
  label?: string;
}

export function Stepper({ value, onChange, min = 0, max = 999, suffix, className, label }: StepperProps) {
  ...
  return (
    <div className={cn("inline-flex items-center gap-3", className)} role="group" aria-label={label}>
      ...
      <span className="min-w-[3.5rem] text-center font-mono text-lg font-bold tabular-nums" aria-live="polite">
        {value}
